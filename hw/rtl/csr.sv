...
module csr (
    ...
    // Field: csr.DG_ENABLE.enable
    always_comb begin
        automatic logic [0:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.DG_ENABLE.enable.value;
        load_next_c = '0;
        if(decoded_reg_strb.DG_ENABLE && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.DG_ENABLE.enable.value & ~decoded_wr_biten[0:0]) | (decoded_wr_data[0:0] & decoded_wr_biten[0:0]);
            load_next_c = '1;
        end
        field_combo.DG_ENABLE.enable.next = next_c;
        field_combo.DG_ENABLE.enable.load_next = load_next_c;
    end
    always_ff @(posedge clk) begin
        if(~rst_n) begin
            field_storage.DG_ENABLE.enable.value <= 1'h0;
        end else begin
            if(field_combo.DG_ENABLE.enable.load_next) begin
                field_storage.DG_ENABLE.enable.value <= field_combo.DG_ENABLE.enable.next;
            end
        end
    end
    assign hwif_out.DG_ENABLE.enable.value = field_storage.DG_ENABLE.enable.value;
    // Field: csr.DG_LENGTH.length
    always_comb begin
        automatic logic [31:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.DG_LENGTH.length.value;
        load_next_c = '0;
        if(decoded_reg_strb.DG_LENGTH && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.DG_LENGTH.length.value & ~decoded_wr_biten[31:0]) | (decoded_wr_data[31:0] & decoded_wr_biten[31:0]);
            load_next_c = '1;
        end
        field_combo.DG_LENGTH.length.next = next_c;
        field_combo.DG_LENGTH.length.load_next = load_next_c;
    end
    always_ff @(posedge clk) begin
        if(~rst_n) begin
            field_storage.DG_LENGTH.length.value <= 32'h0;
        end else begin
            if(field_combo.DG_LENGTH.length.load_next) begin
                field_storage.DG_LENGTH.length.value <= field_combo.DG_LENGTH.length.next;
            end
        end
    end
