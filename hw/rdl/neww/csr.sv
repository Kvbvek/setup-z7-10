...
module csr (
    ...
    always_comb begin
        ...
        is_invalid_rw = '0;
        decoded_reg_strb.INCREMENT = cpuif_req_masked & (cpuif_addr == 3'h0);
    ...
    // Field: csr.INCREMENT.increment
    always_comb begin
        automatic logic [31:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.INCREMENT.increment.value;
        load_next_c = '0;
        if(decoded_reg_strb.INCREMENT && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.INCREMENT.increment.value & ~decoded_wr_biten[31:0]) | (decoded_wr_data[31:0] & decoded_wr_biten[31:0]);
            load_next_c = '1;
        end
        field_combo.INCREMENT.increment.next = next_c;
        field_combo.INCREMENT.increment.load_next = load_next_c;
    end
    always_ff @(posedge clk) begin
        if(~rst_n) begin
            field_storage.INCREMENT.increment.value <= 32'h0;
        end else begin
            if(field_combo.INCREMENT.increment.load_next) begin
                field_storage.INCREMENT.increment.value <= field_combo.INCREMENT.increment.next;
            end
        end
    end
    assign hwif_out.INCREMENT.increment.value = field_storage.INCREMENT.increment.value;
    ...
    assign readback_array[0][31:0] = (decoded_reg_strb.INCREMENT && !decoded_req_is_wr) ? field_storage.INCREMENT.increment.value : '0;
